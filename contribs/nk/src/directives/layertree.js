goog.provide('nk.LayertreeController');
goog.provide('nk.layertreeDirective');
...
nkModule.value('nkLayertreeTemplate',
    /**
     * @param {angular.JQLite} element Element.
     * @param {angular.Attributes} attrs Attributes.
     */
    function(element, attrs) {
      var subTemplateUrl = nk.baseTemplateUrl + '/layertree.html';
      return '<div ngeo-layertree="nkLayertreeCtrl.tree" ' +
          'ngeo-layertree-map="nkLayertreeCtrl.map" ' +
          'ngeo-layertree-nodelayer="nkLayertreeCtrl.getLayer(node)" ' +
          'ngeo-layertree-listeners="nkLayertreeCtrl.listeners(treeScope, ' +
          'treeCtrl)" ' +
...
// the root node that path is defined by the nkLayertreeTemplate value.
ngeoModule.value('ngeoLayertreeTemplateUrl',
     ...
     */
    function(element, attrs) {
      return nk.baseTemplateUrl + '/layertree.html';
    });
 ...
 * 'nk.layertreeTemplateUrl' but you will must adapt the
 * ngeoLayertreeTemplateUrl value too (to define the children's nodes template
 * path).
 *
 * @example
 * <nk-layertree
 *   nk-layertree-source="ctrl.source"
 *   nk-layertree-map="ctrl.map"
 * </nk-layertree>
 *
 * You can add an attribute 'nk-layertree-openlinksinnewwindow="true"' to open
 * metadata URLs in a new window. By default, and in the default template,
 * links will be opened in a popup.
 *
 * @param {string|function(!angular.JQLite=, !angular.Attributes=)}
 *     nkLayertreeTemplate Template for the directive.
 * @return {angular.Directive} The directive specs.
 * @ngInject
 * @ngdoc directive
 * @ngname nkLayertreeDirective
 */
nk.layertreeDirective = function(nkLayertreeTemplate) {
  return {
    scope: {
      'map': '=nkLayertreeMap',
      'tree': '=nkLayertreeSource',
      'openLinksInNewWindowFn': '&nkLayertreeOpenlinksinnewwindow'
    },
    bindToController: true,
    controller: 'nkLayertreeController',
    controllerAs: 'nkLayertreeCtrl',
    template: nkLayertreeTemplate
  };
};

nkModule.directive('nkLayertree', nk.layertreeDirective);
 ...
 * @param {string} nkWmsUrl URL to the wms service to use by default.
 * @constructor
 * @export
 * @ngInject
 * @ngdoc controller
 * @ngname nkLayertreeController
 */
nk.LayertreeController = function($http, $sce, ngeoCreatePopup,
    ngeoLayerHelper, nkWmsUrl) {
 ...
 * @param {nkThemesNode} node Layer tree node.
 * @return {ol.layer.Base} The OpenLayers layer or group for the node.
 * @export
 */
nk.LayertreeController.prototype.getLayer = function(node) {
  var layer;
  var layerName = node.name;
  var layerURL = node.url || this.nkWmsUrl_;
...
/**
 * Remove layer from map and from this nk-layertree's existing layers when the
 ...
 * @export
 */
nk.LayertreeController.prototype.listeners = function(scope, treeCtrl) {
  ...
  scope.$on('$destroy', angular.bind(treeCtrl, function() {
    ...
    // Remove the treeCtrl.layer from nk layertree's layers.
 ...
 * @export
 */
nk.LayertreeController.prototype.getResolutionStyle = function(node) {
  var style;
 ...
 * @export
 */
nk.LayertreeController.prototype.toggleActive = function(treeCtrl) {
 ...
 * @export
 */
nk.LayertreeController.prototype.getNodeState = function(treeCtrl) {
  var style;
 ...
 * @export
 */
nk.LayertreeController.prototype.getLegendIconURL = function(treeCtrl) {
  var layer = treeCtrl.layer;
  var node = treeCtrl.node;
 ...
 * @export
 */
nk.LayertreeController.prototype.getLegendURL = function(treeCtrl) {
  var layer = treeCtrl.layer;
  var node = treeCtrl.node;
 ...
 * nk layerHelper service.
 * @return {?string} The legend URL or null.
 * @private
 */
nk.LayertreeController.prototype.getWMTSLegendURL_ = function(layer) {
 ...
 * @private
 */
nk.LayertreeController.prototype.getWMSLegendURL_ = function(layer,
    opt_legendRule) {
 ...
 * @private
 */
nk.LayertreeController.prototype.getScale_ = function() {
 ...
 * @export
 */
nk.LayertreeController.prototype.displayMetadata = function(treeCtrl) {
 ...
 * @export
 */
nk.LayertreeController.prototype.getNoSourceStyle = function(treeCtrl) {
  var layer = treeCtrl.layer;
 ...
 * @export
 */
nk.LayertreeController.prototype.zoomToResolution = function(node) {
  var view = this.map.getView();
...
nkModule.controller('nkLayertreeController', nk.LayertreeController);
